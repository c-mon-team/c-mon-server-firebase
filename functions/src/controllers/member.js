...
const createMemberChoice = async (req, res) => {
  const { code, userName, choice } = req.body;

  if (!code || !userName || !choice) return res.status(statusCode.BAD_REQUEST).send(util.fail(statusCode.BAD_REQUEST, responseMessage.NULL_VALUE));
  let client;
  try {
    client = await db.connect(req);

    const data = await memberService.createMemberChoice(client, code, userName, choice);
...
const deleteMemberChoice = async (req, res) => {
  ...
  try {
    ...
    const data = await memberService.deleteMemberChoice(client, id);
    res.status(statusCode.OK).send(util.success(statusCode.NO_CONTENT, responseMessage.SUCCESS, null));
  } catch (error) {
    console.log(error);
    res.status(statusCode.INTERNAL_SERVER_ERROR).send(util.fail(statusCode.INTERNAL_SERVER_ERROR, responseMessage.INTERNAL_SERVER_ERROR));
  } finally {
